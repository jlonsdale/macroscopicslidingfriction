...
class Friction {
    constructor(surface1, surface2, k = 1) {
        this.surface1 = surface1;
        this.surface2 = surface2;

        this.areaWeights1 = surface1.areaWeights;
        this.areaWeights2 = surface2.areaWeights;
        this.ndfSamples1 = surface1.ndfSamples;
        this.ndfSamples2 = surface2.ndfSamples;
        this.normals1 = surface1.normals;
        this.normals2 = surface2.normals;
    ...
    getMuAtAngle(angle) {
        ...
        // Normalize angle to [0, 2π) range (input is in radians)
        const normalizedAngle =
            ((angle % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);
        const index = Math.floor(
            (normalizedAngle / (2 * Math.PI)) *
                this.directionalProfileCache.angles.length
        );
        // Clamp index to valid range as safety measure
        const clampedIndex = Math.max(
            0,
            Math.min(index, this.directionalProfileCache.angles.length - 1)
        );
        return this.directionalProfileCache.mus[clampedIndex];
    }
